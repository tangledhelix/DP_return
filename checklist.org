#+title: Guiguts PP Process Checklist

(Updated for [[https://github.com/DistributedProofreaders/guiguts-py][Guiguts 2]])

"The return of the Jews to England" by Henriques, H. S. Q.

* Things to revisit
- [X] Old-fashioned (middle English?) spellings in various places
- [X] Roman numerals not terminating in ~.~ - some terminate in ~,~ and one terminates in ~'s~
- [X] "Dual-calendar dates with a forward slash please - e.g. 1654/5"
  - Need to evaluate this and handle anything here (if needed)
  - [X] p037: blockquote heading using fraction-style markup
  - [X] appears in other places; do a search like ~\d/\d~ to find them.
- [X] There are continuation sidenotes throughout the book.
- [X] Book contains errata (single item at the very end)
- Alignment block items:
  - [X] f003: poetry and right-align (and other front matter)
  - [X] f005: right-align (and other front matter)
  - [X] f006: table
  - [X] f007-f008: table
  - [X] p037: right-align in blockquote
  - [X] p038: poetry
  - [X] p041: right-align in blockquote
- [X] p129-p132: index
- [X] Revisit side notes. So far, rejoined them, moved above paras.
  - https://www.pgdp.net/wiki/Sidenotes
  - put them in the checklist post-split.
- [X] (html) f003: poem with latin?
- [X] p085: ~March 16-May 17~: should be en-dash? Any other cases?
- [X] (html) p007-p008: latin?
- [X] (html) p010-p011: latin?
- [X] (html) p016: latin?
- [X] (html) p018: latin?
- [X] (html) p038: poem with latin?
- [X] (html) p080: latin or french?
- [X] (html) p085-p086: latin?
- [X] (html) footnote 18: latin?
- [X] (html) footnote 22: latin?
- [X] (html) footnote 49: latin?
- [X] (html) footnote 96: ~penc̃on~ is what language? non-italic text. uses non-composed character
  - It's apparently English, some sort of weird contraction of ~pension~ or ~pencion~ where the character stands in for ~ci~ or ~si~. Nothing to tag.
- [X] (html) link ~(note)~ entries in the index instead of using pageno links. (Remove link from page number?)
- [X] (html) add page/note links from tables at start of book
- [X] (html) place mid-paragraph sidenotes
- [X] Add to TN (both editions) something about the x/y dates (day-of-month, and year)
- [X] (html) abbr.json: add ~Car.~ for ~Charles~
- [X] (html) abbr.json: add ~Will.~ for ~William~
- [X] (html) abbr.json: add ~Jac.~ for ~James~
- [X] (html) abbr.json: add ~Vict.~ for ~Victoria~
- [X] (html) abbr.json: add ~Edw.~ for ~Edward~
- [X] (html) abbr.json: add ~Rich.~ for ~Richard~
- [X] (html) abbr.json: add ~Hen.~ for ~Henry~
- [X] (html) resolve meaning of ~c.~ in legal citations (probably ~chapter~)
- [X] (html) resolve meaning of ~stat.~ in legal citations (sometimes ~st.~?) == ~statute~
- [X] (html) abbr.json: add ~sess.~ for ~session~
- [X] (html) abbr.json: add ~cap.~ for ~chapter~
- [X] (html) abbr.json: add ~sect.~ for ~section~
- [X] (html) abbr.json: add ~do.~ for ~ditto~ (sometimes ~Do.~)
- [X] (html) abbr.json: add ~id.~ (Latin ~idem~ ("same")) to the title-less Latin abbrs?
- [X] (html) abbr.json: add ~pt.~ for ~part~
- [X] (html) abbr.json: add ~App.~ for ~Appendix~?
- [X] (html) abbr.json: add ~fol.~ and ~fo.~ for ~folio~?
- [X] (html) abbr.json: add ~Phil.~  for ~Philip~
- [X] (html) abbr.json: add ~Mar.~  for ~Mary~
- [ ] (html) Figure out where ~<main>~, ~<nav>~, ~<section>~ should be ...
* Transcriber Notes
** Leading TN
- Some inconsistencies in spelling, hyphenation, and punctuation have been retained. In particular, archaic and alternate spellings have been preserved, especially those occuring in quotations, e.g., “forein” for “foreign” or the variant spellings “Duarto en Rigis” and “Duarto an Rigij”.
- Obvious printer errors were silently corrected. For more detail about specific corrections to the text, see the Transcriber’s Note at the end of the book.
- (text) Include note about ~_italics_~ & smallcaps (ALLCAPS)
- (text) The ~^~ or ~^{...}~ notation indicates a superscript
- (text) Diamond character ♦ indicates a sidenotes; sometimes inline in a paragraph, sometimes before paragraph.
- Note on dates: “1684/5” style
  - Dates from this period sometimes carry two numbers separated by a slash, affecting the year or the day.
  - Year (e.g., “February 6, 1684/5”): Before 1752, England’s legal year began March 25, not January 1. Dates between January 1 and March 24 could carry both the Old Style civil year (1684) and the modern equivalent (1685).
  - Day (e.g., “March 17/27”): The Julian (Old Style) calendar used in England ran behind the Gregorian (New Style) calendar used on the Continent—by 10 days in the 17th century, 11 in the 18th. The first number is the English date; the second, the Continental equivalent.
** Trailing TN
- (See also the Transcriber’s Note at the beginning of the book.)
- The erratum from the original book has been applied to the text.
- Footnote 94: changed “6 & 7” to “7 & 8” (per erratum). (7 & 8 Vict., cap. 66, sec. 5)
- p. 75: changed “user” to “use” (such use was wholly illegal)
* Notes
** General notes
** Proofer notes
** Smooth reader notes
...
** Project comments
#+begin_src markdown
Alignment . . . . dots (front matter) should be removed. Six spaces preferred.

Dual-calendar dates with a forward slash please - e.g. 1654/5, and A.D. without a space.

----

Images from [TIA](https://archive.org/details/returnofjewstoen00henruoft).
#+end_src
...
- Run the [[https://github.com/tangledhelix/dp_pp_utils][setup script]] to fetch the project resources and create the [[https://github.com/tangledhelix?tab=repositories&q=DP_&type=&language=&sort=][Github]] project
#+begin_src shell
cd ~/dp/util
. venv/bin/activate
./make_project.py
#+end_src
- [X] Read the [[https://www.pgdp.net/c/project.php?id=projectID65b196d0e30c8#project-comments][project comments]]
- [X] Subscribe to the [[https://www.pgdp.net/phpBB3/viewtopic.php?t=81042][forum topic]] and read all posts
- [X] Open ~return-utf8.txt~ in Guiguts
...
- [X] Look for missed diacritical markup. Use Regex Library file ~diacritical_marks.json~.
...
- If specific [[https://www.pgdp.net/wiki/PPTools/Guiguts/Guiguts_Manual/Tools_Menu#Rewrap_Markers][rewrap markers]] should be used, don't change them now, but add a note to your list for later handling, after page separators are removed.
...
- [X] Use the ~Search~ menu to step through all ~/* */~ blocks.
  - Regex: ~^(/\*|\*/)~
  - Apply specific [[https://www.pgdp.net/wiki/PPTools/Guiguts/Guiguts_Manual/Tools_Menu#Table_Indent][indent value]] if desired
  - Make sure poetry line numbers are at least two spaces to the right of the line.
- [X] Use the ~Search~ menu to step through all ~/#..#/~ blocks.
  - Regex: ~^(/#|#/)~
  - Check consistent indentation of block text
  - Apply specific [[https://www.pgdp.net/wiki/PPTools/Guiguts/Guiguts_Manual/Tools_Menu#Block_Quote_Indent_and_Margins][margin values]] if desired
...
- [X] If the to-do list has any notes about adding [[https://www.pgdp.net/wiki/PPTools/Guiguts/Guiguts_Manual/Tools_Menu#Rewrap_Markers][rewrap markers]] (such as indexes or poetry), make those changes.
...
- [X] Click ~Ligatures~. Scan list for [[https://www.pgdp.net/wiki/Æ_and_œ_ligatures][incorrect or inconsistent use]] of ~ae~ and ~oe~ ligatures.
#+begin_src text
...
- [X] If any languages known to be in the book aren't in the Lang: setting, add them (e.g. ~en+fr~). Only tag languages we have a dictionary for. (More dictionaries are available; see [[https://www.pgdp.net/wiki/PPTools/Guiguts/Guiguts_2_Manual/Tools_Menu#Spelling][the wiki]].)
...
- [X] If the book has poetry that uses line numbers, read [[https://www.pgdp.net/wiki/PPTools/Guiguts/Fixup#Poetry_Line_Numbers][this page]] and align the line numbers consistently.
*** Curly quotes
- [X] Curly quotes are [[https://www.pgdp.net/phpBB3/viewtopic.php?f=3&t=73290][recommended]] in both the text and HTML versions. Now is the time to put them in, before the split.
...
- [X] [[https://www.pgdp.net/wiki/En-dash][En dash]]:
...
- [X] Use ~File → Save a Copy As` to make `return.html~
...
- [X] Re-open ~return-utf8.txt~ (if not still open).
*** Convert ~<tb>~, Italic, Bold, and Smallcap
- [X] ~Text → Convert Markup~:
  - [X] Convert [[https://www.pgdp.net/wiki/DP_Official_Documentation:Formatting/Formatting_Guidelines#Formatting_at_the_Character_Level:][inline formatting]].
  - [X] Convert [[https://www.pgdp.net/wiki/DP_Official_Documentation:Formatting/Formatting_Guidelines#Thought_Breaks_.28Extra_Spacing.2FDecoration_Between_Paragraphs.29][thought breaks]].
  - [X] Convert [[https://www.pgdp.net/wiki/DP_Official_Documentation:PP_and_PPV/Guide_to_smallcaps][smallcaps]].
*** Fix ASCII Tables
...
*** Process sidenotes
- [X] For paragraphs with just one sidenote, position it before the paragraph.
- [X] For paragraphs with multiple sidenotes, will need to decide how to handle them. See the [[https://www.pgdp.net/wiki/Sidenotes][wiki article]], which talks about using Unicode diamond characters, among other ideas.
*** Rewrap and Clear Rewrap Markers
- [X] Search for ~^/[*$pcrflxi]~ (regex, no match-case) and make any desired [[https://www.pgdp.net/wiki/PPTools/Guiguts/Guiguts_Manual/Tools_Menu#Rewrap_Markers][rewrap marker]] changes before wrapping
...
- [X] Search ~[Footnote~ (no regex; add 2 leading spaces) to find footnotes in blockquotes; move them out of the blockquote.
...
- [X] Search ~[^A-Z][A-Z]\.\n[A-Z]\.~ to look for initials across a line-break and rewrap as appropriate. (/optional/)
*** Check revisit list
...
- [X] Open ~return.html~ that was saved previously.
...
References: ([[https://www.pgdp.net/phpBB3/viewtopic.php?p=1347865#p1347865][p1347865]]), ([[https://www.pgdp.net/phpBB3/viewtopic.php?p=1179008#p1179008][p1179008]])
- [X] [[https://www.pgdp.net/wiki/Em-dash][Em dash]]:
  - ~Tools → Regex Library~ and load the ~dashes.json~ file
...
- [X] Search for ~^/[*$pcrflxi]~ (regex, no match-case) and make any desired [[https://www.pgdp.net/wiki/PPTools/Guiguts/Guiguts_Manual/Tools_Menu#Rewrap_Markers][rewrap marker]] changes before HTML generation
...
- [X] Page through the book looking for text that was not handled well by automatic HTML generation, in particular:
  - [[https://www.pgdp.net/wiki/DP_Official_Documentation:PP_and_PPV/DP_HTML_Best_Practices/Case_Studies/Title_Pages][Title pages]].
  - [[https://www.pgdp.net/wiki/DP_Official_Documentation:PP_and_PPV/DP_HTML_Best_Practices/Case_Studies/Tables][Tables and Tables of Contents]]. The ~Auto Table~ button can help format tables.
  ...
  - [[https://www.pgdp.net/wiki/Indexes][Indexes]] - can use Guiguts ~/i ... i/~ feature
...
- [X] Hyperlink page references in text, TOC, and index (discussed [[https://www.pgdp.net/wiki/PPTools/Guiguts/HTML#Hyperlinking_Page_Numbers][here]] and [[https://www.pgdp.net/wiki/Indexes][here]]).
- [X] Remove the [[https://www.pgdp.net/wiki/PPTools/Guiguts/Guiguts_Manual/HTML_Menu#Generated_TOC][Generated TOC]] if it is not needed.
...
- [X] Semantic fixup for italics
  ...
  - [[http://www.w3schools.com/tags/ref_language_codes.asp][List of language codes]]
- [X] Using Regex Library and my ~abbr.json~ file, add ~abbr~ tags as appropriate. ([[https://www.pgdp.net/wiki/Accessibility_Recipes/Abbreviations][Reference]])
*** Table notes
...
- [[https://raw.githubusercontent.com/tangledhelix/DP_eugenics/refs/heads/main/images/bottom-brace.png][images/bottom-brace.png]]
- [[https://raw.githubusercontent.com/tangledhelix/DP_eugenics/refs/heads/main/images/left-brace.png][images/left-brace.png]]
- [[https://raw.githubusercontent.com/tangledhelix/DP_eugenics/refs/heads/main/images/right-brace.png][images/right-brace.png]]
- [[https://raw.githubusercontent.com/tangledhelix/DP_eugenics/refs/heads/main/images/top-brace.png][images/top-brace.png]]
...
#+end_src
*** Process sidenotes
- [X] Does Guiguts auto-process these somehow from ~[Sidenote: ]~ markup?
- [X] Decide how to handle these; see the [[https://www.pgdp.net/wiki/Sidenotes][wiki page]].
*** Drop caps
Drop-cap CSS is in my custom header; markup looks like:
#+begin_src html
<p class="drop-cap"><span class="smcap uppercase">The</span>
#+end_src
- [X] Handle any drop-caps ([[https://www.pgdp.net/wiki/DP_Official_Documentation:PP_and_PPV/Easy_Epub/Dropcaps][wiki]])
*** Process Hi-resolution Images
If the project manager provided high-resolution scans of the images in the text, use an image processing program such as GIMP or Adobe Photoshop Elements to optimize them—see [[https://www.pgdp.net/wiki/Guide_to_Image_Processing][Guide to Image Processing]]. You can do this before, during, or after HTML conversion.
...
Taken from a [[https://www.pgdp.net/phpBB3/viewtopic.php?p=1354798&sid=db2ca383a2ce4e9d70bc598253bb0936#p1354798][forum post]]:
#+begin_quote
DO
...
Image [[https://www.pgdp.net/phpBB3/viewtopic.php?f=3&t=70286][sizes]]:
...
- Covers: see [[https://www.pgdp.net/wiki/PP_guide_to_cover_pages][cover documentation]]. Recommend 1600x2560, aspect ratio ~1:1.6. Not over 5000x5000px. Minimum 650x1000. *No specific file size limit*, but use judgement and don't make it larger than necessary.
...
- [X] Use the [[https://www.pgdp.net/wiki/Guide_to_Image_Processing#Linear_Light_in_The_GIMP][dodge/burn layer technique]] to clean up, at least for line drawings
- [X] Resize if needed.
- [X] Sharpen.
...
- [X] If fabricating your own cover, add the TN as noted in [[https://www.pgdp.net/wiki/DP_Official_Documentation:PP_and_PPV/Easy_Epub/Cover][Easy_Epub/Cover]].
...
- [ ] Use ~HTML → CSS Validator~. Fix any reported problems.
  - Validate as CSS 2.1
  - CSS3 is acceptable if current status is ~REC~ on [[https://www.w3.org/Style/CSS/current-work][this page]]
    - Look up the CSS on MDN to figure out which module it comes from; then check that module's status
    - +e.g. CSS3 drop-caps+ CSS3 drop-caps are no longer an issue, no need to leave a note for WW
...
~python3 -m http.server~. Then navigate to ~localhost:8000~.
...
- [ ] Scroll through in multiple browsers (Safari, Chrome, Firefox, etc.)
...
- Renaming epub3 to end in ~.kepub.epub~ improves rendering a lot but it still has weird issues, so it's not really a good solution.
- Also a utility ~kepubify~ (link below) that converts; it's unclear what this conversion does that's any better than just renaming the file. Changing the filename is enough to invoke a different / better rendering engine on Kobo devices.
  - PG is looking at doing KEPUB files, but using Calibre, not kepubify.
- Google Play Books seems to ignore page break CSS entirely?
*** Build Ebook, initial cursory review
- [ ] HTML → Ebookmaker (online) - build EPUB2 and EPUB3
- [ ] Review in Apple Books; focus on page breaks and major issues
** Build and upload ebooks
- [ ] If any changes were made from previous step, rebuild EPUB3
- [ ] Optional: Convert EPUB3 to KEPUB with Calibre
- [ ] Optional: Upload EPUB3 with [[https://www.amazon.com/gp/sendtokindle][Send to Kindle]]
- [ ] Add epub3 to Apple Books
- [ ] Optional: Add EPUB3, KEPUB to Dropbox for Kobo, Android, etc
*** Ebook review
...
- [ ] Go to [[https://www.pgdp.net/c/project.php?id=projectID65b196d0e30c8][project page]], select SR time period, upload ~return-sr.zip~
...
- [ ] Add *anonymized* files to git, e.g. ~return-smoothread01.txt~
...
Ref: [[https://www.pgdp.net/wiki/DP_Official_Documentation:PP_and_PPV/Guide_to_Direct_Uploading_%28DU%29_and_Posting_to_PG][Guide to DU and Posting to PG]]
- [ ] ~make zip~
- [ ] Run through [[https://ebookmaker.pglaf.org/][ebookmaker]], check output.txt
  - No errors!
  - Warnings are OK
  - Copy link to the output.txt file (will persist about 3 days)
- [ ] Go to the [[https://upload.pglaf.org/][PGLAF upload site]]
- [ ] Enter copyright clearance (get from DP [[https://www.pgdp.net/c/project.php?id=projectID65b196d0e30c8][project page]])
...
- [ ] Credits line
  - Paste in from DP [[https://www.pgdp.net/c/project.php?id=projectID65b196d0e30c8][project page]]
...
- For projects over a month old, use the [[https://www.gutenberg.org/help/errata.html][errata process]]
- If not posted within a week, contact WWers pgww at lists.pglaf.org
** Project wrap-up
- [ ] When posted, then update PG URL in README.md, and [[https://tangledhelix.com/about][personal website list]], and set homepage on GitHub project
...
** Related Pages
- [[https://www.pgdp.net/wiki/Beginner_PP_advice][Beginner PP advice]]
- [[https://www.pgdp.net/c/faq/post_proof.php][Post-Processing FAQ]]
- [[https://www.pgdp.net/wiki/Post-Processing_Advice][Post-Processing Advice]]
- [[https://www.pgdp.net/wiki/Guiguts_Tutorial][Guiguts Tutorial]]
- [[https://www.pgdp.net/wiki/PPTools/Guiguts][Guiguts Manual]]
- [[https://www.pgdp.net/wiki/Getting_your_PP_Project_Ready_for_PPV][Getting your PP Project Ready for PPV]]
- [[https://www.pgdp.net/wiki/DP_Official_Documentation:PP_and_PPV/DP_HTML_Best_Practices/Case_Studies/Poetry][Poetry Case Study]]
